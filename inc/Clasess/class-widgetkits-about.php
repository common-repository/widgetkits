<?php
/**
 * Widgetkit class-widgetkits-about.php widget class file
 *
 * @package Widgetkit
 */
...
class WidgetkitsAbout extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			esc_html__( '= Widgetkits = About', 'widgetkits' ), // Name
			array( 'description' => esc_html__( 'Display Author About Info', 'widgetkits' ) ) // Args
		);
	...
	public function enqueue_scripts($hook) {
		...
		wp_enqueue_media();
		wp_enqueue_script( 'widgetkits-widget-js', WIGETKITS_ASSETS_PUBLIC . '/js/sidebar-widget.js',  array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ),WIGETKITS_VERSION, true );
	...
	public function widget( $args, $instance ) {
		$wbg_color = $instance['wk_bg_color'];
    	if ( empty($wbg_color) ){ $wbg_color = "#fff"; }

		echo wp_kses_post( $args['before_widget'] );
		$wk_bg_color = ! empty( $instance['wk_bg_color'] ) ? $instance['wk_bg_color'] : __( '#fff', 'widgetkits' );
		...
		<div class="widgetkits-about-wrap">
			...
			<div class="widgetkits-author-content">
				...
				<?php endif; ?>
				<div class="widgetkits-about-content"  style="background: <?php echo esc_attr($wbg_color) ?>; box-shadow: 0px 10px 40px rgba(0, 0, 0, 0.04);">
				...
				<?php if ( ! empty( $description ) && ! empty( $description ) ) : ?>
					<p class="text-widgetkits-manatee"><?php echo wp_kses_post( $description ); ?></p>
				<?php endif ?>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['wk_bg_color'] = ( ! empty( $new_instance['wk_bg_color'] ) ) ? strip_tags( $new_instance['wk_bg_color'] ) : '';
	...
	public function form( $instance ) {
		...
		$wk_bg_color = isset($instance['wk_bg_color']) ? $instance['wk_bg_color'] : __( '#fff', 'widgetkits' );
		$title  = isset( $instance['title'] ) ? $instance['title'] : '';
		$thumb        = isset( $instance['thumb'] ) ? $instance['thumb'] : '';
		$description  = isset( $instance['description'] ) ? $instance['description'] : '';
		$social_links = ( ! empty( $instance['social_links'] ) ) ? $instance['social_links'] : array();
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Author Name:', 'widgetkits' ); ?></label>
		...
		<p>
	      <label for="<?php echo $this->get_field_id( 'wk_bg_color' ); ?>" style="display:block;"><?php _e( 'Background Color', 'widgetkits' ); ?></label>
	      <input class="widefat color-picker" id="<?php echo $this->get_field_id( 'wk_bg_color' ); ?>" name="<?php echo $this->get_field_name( 'wk_bg_color' ); ?>" type="text" value="<?php echo esc_attr( $wk_bg_color ); ?>" />
	    </p>
