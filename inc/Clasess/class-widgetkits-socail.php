<?php
/**
 * Widgetkit class-widgetkits-socail.php widget class file
 *
 * @package Widgetkit
 */

/**
 * Widgetkit social widget.
 */
class Widgetkitssocial extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'widgetkits_socail', // Base ID
			esc_html__( '= Widgetkits = Social', 'widgetkits' ), // Name
			array( 'description' => esc_html__( 'Display Social Links', 'widgetkits' ) ) // Args
		);
	...
	public function widget( $args, $instance ) {
		...
		?>
		<div class="widgetkits-socail-wrap">
			<?php if ( ! empty( $title ) && ! empty( $title ) ) : ?>
				<h3 class="widget-title m-bottom-40"><?php echo esc_html( $title ); ?></h3>
			<?php endif ?>
				...
				<ul class="list-inline m-0">
					...
					foreach ( $social_links as $info ) :
						if ( ! empty( $info['thumb'] ) ) :
							?>
							<li>
								<a target="blank" href="<?php echo ( ! empty( $info['title'] ) ) ? esc_url( $info['title'] ) : '#'; ?>" title="<?php echo esc_attr( $info['title'] ); ?>"><i class="<?php echo esc_attr( $info['thumb'] ); ?>" aria-hidden="true"></i></a></li>
							<?php
						endif;
					endforeach;
					?>
	...
	public function form( $instance ) {
		...
		<div class="widgetkits-repeater-info-wrapper" data-id-name="widgetkits_socail" data-field-id="social_links" data-placeholder="<?php esc_attr_e("Social Page Url", "widgetkits"); ?>">
...
} // class Widgetkitsocial




if ( ! function_exists( 'widgetkits__socail_widget_init' ) ) {
	function widgetkits__socail_widget_init() {
		register_widget( 'Widgetkitssocial' );
	}
}
add_action( 'widgets_init', 'widgetkits__socail_widget_init' );
